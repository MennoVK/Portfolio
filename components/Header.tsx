...
export const Header = () => {
    ...
    return (
        <header className="fixed z-10 flex justify-between w-full gap-10 p-20">
            <Link className={clsx(button({ rounded: "tl" }), "flex items-center justify-center !text-36 h-fit")} href="/">Menno<br></br> Veerkamp</Link>
            <button className={clsx(button({rounded: "br"}), active && "h-fit bg-black")} onClick={() => {setActive(!active)}}>
                <span className="inline-block">About me</span>
                {active ? 
                    <Image src="/svgs/minus.svg" alt="minus" width={11} height={11} className="inline mx-10 align-baseline"/>
                    :
                    <Image src="/svgs/plus.svg" alt="plus" width={11} height={11} className="inline mx-10 align-baseline"/>
                }
                {active &&
            ...
            <Link className={clsx(button({ rounded: "tlbr" }), "flex items-center justify-center  h-fit")} href="mailto:contact@mennoveerkamp.com" target="_blank">
